...
const programs: ChedProgram[] = [
  {
    code: 'BSIT-BN',
    title: 'Bachelor of Science in Information Technology',
    image: itImage,
    documentUrl: 'https://drive.google.com/file/d/1WT3YgpytAYV2Wla4r3G73k0CFZvy9-Q6/view?usp=sharing',
  },
  {
    code: 'BSED-SS-BN',
    title: 'Bachelor of Secondary Education Major in Social Studies',
    image: socialStudiesImage,
    documentUrl: 'https://drive.google.com/file/d/18v_nceiMgNVqtPYfx2o5Ito5USTQrGzV/view?usp=sharing',
  },
  {
    code: 'BEED-BN',
    title: 'Bachelor of Elementary Education',
    image: elementaryImage,
    documentUrl: 'https://drive.google.com/file/d/10-5sXr4p2cKy7abBS2e3a9YHoOtr8ROa/view?usp=sharing',
  },
  {
    code: 'BSED-EN-BN',
...
export function ChedMemorandumSection() {
  return (
    <section className="relative overflow-hidden bg-[#f7f5f1] py-16 sm:py-20 lg:py-28" aria-labelledby="ched-memorandum-title">
      ...
      <div className="mx-auto max-w-6xl px-5 lg:px-8">
        <ScrollReveal className="mx-auto max-w-3xl text-center">
          <p className="text-xs font-bold uppercase tracking-[0.24em] text-[#8a1724]">Official CHED documents</p>
          <h2 id="ched-memorandum-title" className="mt-4 text-3xl font-black uppercase tracking-tight sm:text-4xl lg:text-5xl">Memorandum Orders by Program</h2>
          <div className="mx-auto mt-5 h-1 w-16 rounded-full bg-amber-400" />
          <p className="mt-5 leading-7 text-slate-600">Select an academic program to view its applicable CHED memorandum order on Google Drive.</p>
        </ScrollReveal>

        <div className="mt-12 grid gap-6 lg:mt-16">
          {programs.map((program, index) => {
            const isAvailable = Boolean(program.documentUrl)

            return (
              <ScrollReveal key={program.code} delay={(index % 3) * 70}>
                <a
                  href={program.documentUrl ?? '/coming-soon'}
                  target={isAvailable ? '_blank' : undefined}
                  rel={isAvailable ? 'noopener noreferrer' : undefined}
                  aria-label={isAvailable ? `View the ${program.title} CHED memorandum order on Google Drive` : `${program.title} CHED memorandum order coming soon`}
