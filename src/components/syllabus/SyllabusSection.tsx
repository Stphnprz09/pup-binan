...
const programs: SyllabusProgram[] = [
  {
    code: 'BSIT-BN',
    title: 'Bachelor of Science in Information Technology',
    image: itImage,
    folderUrl: 'https://drive.google.com/drive/folders/1p0jQdXBb0jRByEP5RW7FZHtI9WlrQSW4',
  },
  {
    code: 'BSED-SS-BN',
    title: 'Bachelor of Secondary Education Major in Social Studies',
    image: socialStudiesImage,
    folderUrl: 'https://drive.google.com/drive/folders/1vieKPdbRpzGjALp10Xazq9OEVLQE2rLR',
  },
  {
    code: 'BEED-BN',
    title: 'Bachelor of Elementary Education',
    image: elementaryImage,
    folderUrl: 'https://drive.google.com/drive/folders/1xPR1Zeqwg7NG0JQNFCCM05CZW5jM982_',
  },
  {
    code: 'BSED-EN-BN',
    title: 'Bachelor of Secondary Education Major in English',
    image: englishImage,
    folderUrl: 'https://drive.google.com/drive/folders/17LJ0qb4-Gjz8BFF2yiVLFhxJcUuOkZss',
  },
  {
    code: 'BSCPE-BN',
...
export function SyllabusSection() {
  return (
    <section className="relative overflow-hidden bg-[#f7f5f1] py-16 sm:py-20 lg:py-28" aria-labelledby="syllabus-directory-title">
      ...
      <div className="mx-auto max-w-6xl px-5 lg:px-8">
        <ScrollReveal className="mx-auto max-w-3xl text-center">
          <p className="text-xs font-bold uppercase tracking-[0.24em] text-[#8a1724]">Academic course documents</p>
          <h2 id="syllabus-directory-title" className="mt-4 text-3xl font-black uppercase tracking-tight sm:text-4xl lg:text-5xl">Syllabi by Program</h2>
          <div className="mx-auto mt-5 h-1 w-16 rounded-full bg-amber-400" />
          <p className="mt-5 leading-7 text-slate-600">Select a program to browse its available syllabus folder. Additional program folders will be published as they become available.</p>
        </ScrollReveal>

        <div className="mt-12 grid gap-6 lg:mt-16">
          {programs.map((program, index) => {
            const isAvailable = Boolean(program.folderUrl)

            return (
              <ScrollReveal key={program.code} delay={(index % 3) * 70}>
                <a
                  href={program.folderUrl ?? '/coming-soon'}
                  target={isAvailable ? '_blank' : undefined}
                  rel={isAvailable ? 'noopener noreferrer' : undefined}
                  aria-label={isAvailable ? `View the ${program.title} syllabus folder on Google Drive` : `${program.title} syllabi coming soon`}
                ...
                >
                  ...
                  <div className="flex min-w-0 flex-col justify-center p-6 sm:p-8">
                    ...
                    <span className="mt-5 inline-flex items-center gap-2 text-sm font-bold text-[#8a1724]">
                      {isAvailable ? 'View folder on Google Drive' : 'View coming soon page'} <LinkIcon external={isAvailable} />
                    </span>
                  </div>
                </a>
              </ScrollReveal>
